...
function package() {
    return packagingService.packageLambda()
        .catch(err => {
            return Promise.reject(`Error occured during packaging: ${err}`);
        });
}
exports.package = package;

function template() {
    return fs.copy(constants.DEFAULT_TEMPLATE_DIRECTORY + '/sdfsd/', constants.USER_TEMPLATE_DIRECTORY)
        .catch(err => {
            console.log(`Error occured copying template files: ${err}`.red);
        });
}
