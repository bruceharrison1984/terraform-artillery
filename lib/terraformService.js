...
const defaultTemplateDirectory = `${__dirname}/lambda_template`;
...
function destroy() {
    isTerraformInstalled()
        .then(() => {
            execSh(`cd ${terraformTemplateDirectory} && terraform destroy --force`, err => {
                    if (err) {
...
function package() {
    return new Promise((resolve, reject) => {
        fs.ensureDir(lambdaPackageDirectory)
            ...
            .then(() => {
                zip(lambdaPackageLocation, ['./lambda_template/handler.js', './lambda_template/node_modules/'], err => {
                    if (err) {
