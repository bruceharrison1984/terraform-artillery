...
function package() {
    let packageLocation = './lambda_package/lambda.zip';
    return new Promise((resolve, reject) => {
        fs.ensureDir('lambda_package').then(() => {
            zip(packageLocation, ['./handler.js', './node_modules/', './package.json'], err => {
                if (err) {
                    reject(`Error occured while creating AWS Lambda package: ${err}`);
                }

                let packageSize = fs.statSync(packageLocation).size / 1000000;
...
function template() {
    return new Promise((resolve, reject) => {
        fs.copy('./lib/lambda_template', './lambda_template', err => {
            if (err) {
