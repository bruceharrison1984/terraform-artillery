...
function destroy() {
    isTerraformInstalled()
        .then(() => {
            execSh(`cd ${terraformTemplateDirectory} && terraform destroy -state=${cwd}/tfstate/terraform.tfstate --force -var 'lambda_package=${lambdaPackageLocation}'`, err => {
                    if (err) {
...
function package() {
    return new Promise((resolve, reject) => {
        fs.ensureDir(lambdaPackageDirectory)
            ...
            .catch( e => (e==='Template folder not changed.') ? false : Promise.reject(e) )
            .then((templateChanged) => {
                if(templateChanged!==false) {
                    return buildPackageZip()
                      .then( () => md5dir(userTemplateDirectory) )
                      .then( (md5) => fs.writeFile(userTemplateHash,md5) )
                      .then(resolve);
                }else{
                    console.log('Template not changed, skipping package creation'.cyan);
                    resolve();
                }
            }).catch(err => {
                console.error(colors.red(err));
...
function isTemplateFolderChanged() {
    return new Promise((resolve,reject) => {
        isTemplateFolderCreated()
            ...
            .then( (results) => {
                ( results[0] !== results[1] )? resolve() : reject('Template folder not changed.');
            } )
            .catch(resolve);
    });
}
...
function getTerraformCommand({ useast1, useast2, uswest1, uswest2, allregions, env, plan }) {
    ...
    terraformCommand += ` -state=${cwd}/tfstate/terraform.tfstate`;
